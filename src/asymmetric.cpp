    ...
    while (h < nsamps)
    {
        ...
        for (int i = 0; i < N_blocks; i++)
        {
            ...
            if (scalar_block)
            {
                ...
                priorcomp = log_asymmetric_prior_scalar(current_beta, vglobal, s, prob_vec(i), penalized_scalar, scale_sigma_prior);
                if (singular == true)
                {
                ...
                {
                    while (log_asymmetric_prior_scalar(beta_hat_scalar + betaprop_scalar, vglobal, s, prob_vec(i), penalized_scalar, scale_sigma_prior) - log_normal_density_scalar_2(beta_hat_scalar + betaprop_scalar, Sigma_scalar_inv_diag, Sigma_scalar_log_det, singular) < ly)
                    {
            ...
            priorcomp = log_asymmetric_prior(b.rows(block_start, block_end), vglobal, s, prob_vec.subvec(i, i), penalize_index_block, all_penalized_block, scale_sigma_prior);
            if (singular == true)
            {
            ...
            {
                while (log_asymmetric_prior(beta_hat_block + betaprop, vglobal, s, prob_vec.subvec(i, i), penalize_index_block, all_penalized_block, scale_sigma_prior) - log_normal_density_matrix_2(beta_hat_block + betaprop, Sigma_block_inv_diag, Sigma_block_log_det, singular) < ly)
                {
