    ...
    */
    ...
    while (h < nsamps)
    {
        ...
        for(int i = 0; i < N_blocks; i ++){
            ...
            priorcomp = log_nonlocal_prior(b.rows(block_indexes(i), block_indexes(i+1)-1), vglobal, penalize.rows(block_indexes(i), block_indexes(i+1)-1), prior_mean.rows(block_indexes(i), block_indexes(i+1)-1)) - log_normal_density_matrix(b.rows(block_indexes(i), block_indexes(i+1)-1), eye<mat>(block_vec(i), block_vec(i)) / pow(s,2) / eta, singular);
            ...
            if(i == 0 && icept == true){
                ...
                while (log_nonlocal_prior(beta_hat_block + betaprop, vglobal, penalize.rows(block_indexes(i), block_indexes(i+1)-1), prior_mean.rows(block_indexes(i), block_indexes(i+1)-1)) - log_normal_density_matrix(beta_hat_block + betaprop, eye<mat>(block_vec(i), block_vec(i)) / pow(s,2) / eta, singular)< ly){
