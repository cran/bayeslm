...
List nonlocal_cpp_loop(arma::mat Y, arma::mat X, arma::vec prior_mean, arma::uvec penalize, arma::vec block_vec, arma::vec cc, int prior_type = 1, double sigma = 0.5, double s2 = 4, double kap2 = 16, int nsamps = 10000, int burn = 1000, int skip = 1, double vglobal = 1.0, bool sampling_vglobal = true, bool verb = false, bool icept = false, bool standardize = true, bool singular = false, bool scale_sigma_prior = true)
{
    ...
    */
    ...
    arma::field<arma::vec> prior_mean_blocks(N_blocks);
    ...
    for (int i = 0; i < N_blocks; i++)
    {
        ...
        prior_mean_blocks(i) = prior_mean.rows(block_start, block_end);
        if (singular == true)
        {
    ...
    while (h < nsamps)
    {
        ...
        for (int i = 0; i < N_blocks; i++)
        {
            ...
            const arma::vec &prior_mean_block = prior_mean_blocks(i);
            ...
            if (scalar_block)
            {
                ...
                priorcomp = log_nonlocal_prior_scalar(current_beta, vglobal, s, prior_mean_block(0), penalized_scalar, scale_sigma_prior);
                if (singular == true)
                {
                ...
                {
                    while (log_nonlocal_prior_scalar(beta_hat_scalar + betaprop_scalar, vglobal, s, prior_mean_block(0), penalized_scalar, scale_sigma_prior) - log_normal_density_scalar_2(beta_hat_scalar + betaprop_scalar, Sigma_scalar_inv_diag, Sigma_scalar_log_det, singular) < ly)
                    {
            ...
            priorcomp = log_nonlocal_prior(b.rows(block_start, block_end), vglobal, s, penalize_index_block, prior_mean_block, all_penalized_block, scale_sigma_prior);
            if (singular == true)
            {
            ...
            {
                while (log_nonlocal_prior(beta_hat_block + betaprop, vglobal, s, penalize_index_block, prior_mean_block, all_penalized_block, scale_sigma_prior) - log_normal_density_matrix_2(beta_hat_block + betaprop, Sigma_block_inv_diag, Sigma_block_log_det, singular) < ly)
                {
