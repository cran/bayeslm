...
List blasso_cpp_loop(arma::mat Y, arma::mat X, arma::uvec penalize, arma::vec block_vec, arma::vec cc, int prior_type = 1, double sigma = 0.5, double s2 = 4, double kap2 = 16, int nsamps = 10000, int burn = 1000, int skip = 1, double vglobal = 1.0, bool sampling_vglobal = true, bool verb = false, bool icept = false, bool standardize = true, bool singular = false, bool scale_sigma_prior = true)
{
    ...
    */
    ...
    double penalized_beta_abs_sum = penalized_abs_sum(b, penalize_index, all_penalized);
    ...
    while (h < nsamps)
    {
        ...
        for (int i = 0; i < N_blocks; i++)
        {
            ...
            const double block_abs_sum_current = penalized_abs_sum(b.rows(block_start, block_end), penalize_index_block, all_penalized_block);
            ...
            if (scalar_block)
            {
                ...
                priorcomp = log_laplace_prior_scalar(current_beta, tau, s, vglobal, penalized_scalar);
                if (singular == true)
                {
                ...
                {
                    while (log_laplace_prior_scalar(beta_hat_scalar + betaprop_scalar, tau, s, vglobal, penalized_scalar) - log_normal_density_scalar_2(beta_hat_scalar + betaprop_scalar, Sigma_scalar_inv_diag, Sigma_scalar_log_det, singular) < ly)
                    {
                ...
                // Keep the Laplace sufficient statistic current so tau updates
                // do not rescan the whole beta vector every iteration.
                penalized_beta_abs_sum += (penalized_scalar ? std::abs(b(block_start)) : 0.0) - block_abs_sum_current;
                continue;
            }
            ...
            priorcomp = log_laplace_prior(b.rows(block_start, block_end), tau, s, vglobal, penalize_index_block, all_penalized_block);
            if (singular == true)
            {
            ...
            {
                while (log_laplace_prior(beta_hat_block + betaprop, tau, s, vglobal, penalize_index_block, all_penalized_block) - log_normal_density_matrix_2(beta_hat_block + betaprop, Sigma_block_inv_diag, Sigma_block_log_det, singular) < ly)
                {
            ...
            penalized_beta_abs_sum += penalized_abs_sum(b.rows(block_start, block_end), penalize_index_block, all_penalized_block) - block_abs_sum_current;
        }

        // tau only depends on the penalized absolute sum, so the cached
        // statistic above makes this Metropolis update O(1).
        tau_prop = exp(log(tau) + arma::as_scalar(randn(1)) * 0.05);

        ratio = exp(log_laplace_prior_from_stats(penalized_beta_abs_sum, penalized_count, tau_prop, s, vglobal) + log_normal_density(tau_prop, 0.0, 100.0) - log_laplace_prior_from_stats(penalized_beta_abs_sum, penalized_count, tau, s, vglobal) - log_normal_density(tau, 0.0, 100.0) + log(tau_prop) - log(tau));
