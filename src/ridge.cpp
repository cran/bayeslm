...
List bridge_cpp_loop(arma::mat Y, arma::mat X, arma::uvec penalize, arma::vec block_vec, arma::vec cc, int prior_type = 1, double sigma = 0.5, double s2 = 4, double kap2 = 16, int nsamps = 10000, int burn = 1000, int skip = 1, double vglobal = 1.0, bool sampling_vglobal = true, bool verb = false, bool icept = false, bool standardize = true, bool singular = false, bool scale_sigma_prior = true)
{
    ...
    */
    ...
    const bool all_penalized = penalize_index.n_elem == penalize.n_elem;
    ...
    double lambda = 2.0;
    const arma::uword penalized_count = all_penalized ? static_cast<arma::uword>(b.n_elem) : static_cast<arma::uword>(penalize_index.n_elem);
    double penalized_beta_sq_sum = penalized_square_sum(b, penalize_index, all_penalized);
    ...
    while (h < nsamps)
    {
        ...
        for (int i = 0; i < N_blocks; i++)
        {
            ...
            const double block_sq_sum_current = penalized_square_sum(b.rows(block_start, block_end), penalize_index_block, all_penalized_block);
            ...
            if (scalar_block)
            {
                ...
                priorcomp = log_ridge_prior_scalar(current_beta, lambda, vglobal, s, penalized_scalar, scale_sigma_prior);
                if (singular == true)
                {
                ...
                {
                    while (log_ridge_prior_scalar(beta_hat_scalar + betaprop_scalar, lambda, vglobal, s, penalized_scalar, scale_sigma_prior) - log_normal_density_scalar_2(beta_hat_scalar + betaprop_scalar, Sigma_scalar_inv_diag, Sigma_scalar_log_det, singular) < ly)
                    {
                ...
                // Keep the ridge sufficient statistic current so lambda updates
                // do not rescan the whole beta vector every iteration.
                penalized_beta_sq_sum += (penalized_scalar ? b(block_start) * b(block_start) : 0.0) - block_sq_sum_current;
                continue;
            }
            ...
            priorcomp = log_ridge_prior(b.rows(block_start, block_end), lambda, vglobal, s, penalize_index_block, all_penalized_block, scale_sigma_prior);
            if (singular == true)
            {
            ...
            {
                while (log_ridge_prior(beta_hat_block + betaprop, lambda, vglobal, s, penalize_index_block, all_penalized_block, scale_sigma_prior) - log_normal_density_matrix_2(beta_hat_block + betaprop, Sigma_block_inv_diag, Sigma_block_log_det, singular) < ly)
                {
            ...
            penalized_beta_sq_sum += penalized_square_sum(b.rows(block_start, block_end), penalize_index_block, all_penalized_block) - block_sq_sum_current;
        }

        // lambda only depends on the penalized square sum, so this Metropolis
        // step can reuse the cached statistic above.
        lambda_prop = exp(log(lambda) + arma::as_scalar(randn(1)) * 0.05);

        ratio = exp(log_ridge_prior_from_stats(penalized_beta_sq_sum, penalized_count, lambda_prop, vglobal, s, scale_sigma_prior) + log_normal_density(lambda_prop, 0.0, 100.0) - log_ridge_prior_from_stats(penalized_beta_sq_sum, penalized_count, lambda, vglobal, s, scale_sigma_prior) - log_normal_density(lambda, 0.0, 100.0) + log(lambda_prop) - log(lambda));
        ...
        if (sampling_vglobal)
        {
            ...
            ratio = exp(log_horseshoe_approx_prior(b, vgprop, s, penalize_index, all_penalized, scale_sigma_prior) - log_horseshoe_approx_prior(b, vglobal, s, penalize_index, all_penalized, scale_sigma_prior) + log(vgprop) - log(vglobal));
